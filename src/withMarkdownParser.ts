...
export function withMarkdownParser<T extends Editor>(
  editor: T
): Editor & ReactEditor {
  const e = editor as T & ReactEditor;
  const { insertData, isInline, isVoid } = e;
  ...
  e.insertData = data => {
    ...
    if (content) {
      ...
      const fragment: Array<Node> = deserialize(parsed.body).filter(
        filterBreaklines
      );
...
function filterBreaklines(item: any): Boolean {
  return !item.text;
}
